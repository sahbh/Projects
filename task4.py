...
# Finding the highest average driver total pay by time_of_day
avg_driver_total_pay = main_df.groupBy("time_of_day").agg(avg("driver_total_pay").alias("average_driver_total_pay")) \
                    .orderBy("average_driver_total_pay", ascending=False) #sorted by highest pay first
avg_driver_total_pay.show()
    
# Finding the average trip length by time_of_day
avg_trip_len = main_df.groupBy("time_of_day").agg(avg("trip_length").alias("average_trip_length")) \
                    .orderBy("average_trip_length", ascending=False) #sorted by highest pay first
avg_trip_len.show()

# Finding the avg earnings per mile by join the last 2 results (average driver pay and average trip length on time_of_day)
avg_earnings_per_mile = avg_driver_total_pay.join(avg_trip_len, "time_of_day") \
                                      .withColumn("average_earning_per_mile", 
