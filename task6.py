...
# Find the trip counts >0 but <1000, group by Pickup_Borough and time_of_day, then count trips. Filter and order for desired format. 
trips = main_df.groupBy("Pickup_Borough", "time_of_day") \
                .agg(count("*").alias("trip_count")) \
                .filter((col("trip_count") > 0) & (col("trip_count") < 1000)) \
                .orderBy("Pickup_Borough", "time_of_day")
trips.show()

# Filter for evening time_of_day and then group by pickup borough and time of day then count trips by Pickup_Borough
eveningtime_trips = main_df.filter(col("time_of_day") == "evening") \
                    .groupBy("Pickup_Borough", "time_of_day") \
                    .agg(count("*").alias("trip_count")) \
                    .orderBy("Pickup_Borough")
eveningtime_trips.show()

# Use filter to find trips from brooklyn to staten island
bk_to_st_isl_trips = main_df.filter((col("Pickup_Borough") == "Brooklyn") & (col("Dropoff_Borough") == "Staten Island"))
print("Total trips from Brooklyn to Staten Island:", bk_to_st_isl_trips.count()) # then trip count
bk_to_st_isl_trips.select("Pickup_Borough", "Dropoff_Borough", "Pickup_Zone").show(10, truncate=False)# Display 10 samples  
